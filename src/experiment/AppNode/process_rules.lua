...
function draw_block_axes(block_position, block_orientation, color)
    ...
    api.debug_arrow(color, block_position,
                    block_position + 0.1 * vector3(z):rotate(block_orientation))
end
    ...
    function range(from, to, step)
        ...
        return function(_, lastvalue)
            ...
            if step > 0 and nextvalue <= to or step < 0 and nextvalue >= to or
                step == 0 then return nextvalue end
        end, nil, from - step
    end
...
function check_block_in_safe_zone(block)
    ...
    camera_position_in_robot = camera_position_in_end_effector +
                                   vector3(0.0980875, 0,
                                           robot.lift_system.position + 0.055)
    ...
    staring_point = vector3(0, 0, c / math.tan(vertical_fov / 2)):rotate(
                        camera_orientation_in_robot) +
                        vector3(camera_position_in_robot)
    api.debug_arrow('green', vector3(staring_point),
                    vector3(x_limit_for_max_z, y_limit_for_max_z, z_limit):rotate(
                        camera_orientation_in_robot) +
                        vector3(camera_position_in_robot))
    api.debug_arrow('green', vector3(staring_point),
                    vector3(-1 * x_limit_for_max_z, y_limit_for_max_z, z_limit):rotate(
                        camera_orientation_in_robot) +
                        vector3(camera_position_in_robot))

    api.debug_arrow('green', vector3(staring_point),
                    vector3(x_limit_for_max_z, -1 * y_limit_for_max_z, z_limit):rotate(
                        camera_orientation_in_robot) +
                        vector3(camera_position_in_robot))

    api.debug_arrow('green', vector3(staring_point),
                    vector3(-1 * x_limit_for_max_z, -1 * y_limit_for_max_z,
                            z_limit):rotate(camera_orientation_in_robot) +
                        vector3(camera_position_in_robot))
    ...
    if block.position_robot.z > 0.12 then -- the block could not be higher
        ...
        if z < z_limit and y < y_limit and y > -1 * y_limit and x < x_limit and
            x > -1 * x_limit then
            -- print('block:', block.id, 'is safe')
            return true
        else
...
function group_blocks()
    ...
    function check_connected(block_1, block_2)
        ...
        orientation_diff = math.sqrt((block_1.orientation_robot.x -
                                         ...
                                         (block_1.orientation_robot.y -
                                             block_2.orientation_robot.y) ^ 2)

        position_diff = math.sqrt((block_1.position_robot.x -
                                      block_2.position_robot.x) ^ 2 +
                                      (block_1.position_robot.y -
                                          block_2.position_robot.y) ^ 2 +
                                      (block_1.position_robot.z -
                                          block_2.position_robot.z) ^ 2)
...
local create_process_rules_node = function(rules, rule_type, final_target)
    ...
    return function()
        ...
        for i, group in pairs(grouped_blocks) do
            ...
            r2_in_r1_pos = vector3(r2_in_b1_pos):rotate(b1_in_r1_ori) +
                               b1_in_r1_pos
            ...
            for j, block in pairs(group) do
                ...
                b_in_r2_pos =
                    vector3(b_in_r1_pos):rotate(r2_in_r1_ori:inverse()) +
                        r1_in_r2_pos
                bj_in_r2_pos[tostring(block.id)] = {}
                bj_in_r2_pos[tostring(block.id)].index =
                    (b_in_r2_pos - b1_in_r2_pos)
                ...
                bj_in_r2_pos[tostring(block.id)].index.x =
                    round(bj_in_r2_pos[tostring(block.id)].index.x / 0.05, 0)
                bj_in_r2_pos[tostring(block.id)].index.y =
                    round(bj_in_r2_pos[tostring(block.id)].index.y / 0.05, 0)
                bj_in_r2_pos[tostring(block.id)].index.z =
                    round(bj_in_r2_pos[tostring(block.id)].index.z / 0.05, 0)
            end
        ...
        function get_reference_id_from_index(reference_index, visible_structure)
            for j, block in pairs(visible_structure) do
                if block.index == reference_index then return j end
            end
        end
        ...
        for i, rule in pairs(rules.list) do
            if rule.generate_orientations ~= nil and rule.generate_orientations ==
                true then
        ...
        for i, rule in pairs(rules.list) do
            ...
            rule.target.reference_index.x =
                round(rule.target.reference_index.x - lowest_x, 0)
            rule.target.reference_index.y =
                round(rule.target.reference_index.y - lowest_y, 0)
            rule.target.reference_index.z =
                round(rule.target.reference_index.z - lowest_z, 0)
        end
        ...
        function one_block_safe(indexed_structure)
            ...
            for bi, indexed_block in pairs(indexed_structure) do
                for b, block in pairs(api.blocks) do
                    if tonumber(get_reference_id_from_index(indexed_block.index,
                                                            indexed_structure)) ==
                        tonumber(block.id) then
                    ...
                    if target_block_safe(visible_structure,
                        ...
                        if res1 == true and res2 == true then
                            ...
                            possible_target.offset =
                                rule.target.offset_from_reference
                ...
                for j, block in pairs(api.blocks) do
                    if tostring(block.id) == possible_target.reference_id then
                        distance_from_target =
                            math.sqrt((block.position_robot.x) ^ 2 +
                                          (block.position_robot.y) ^ 2)
                        if distance_from_target < minimum_distance then
                            minimum_distance = distance_from_target
                            final_target.reference_id =
                                tonumber(possible_target.reference_id)
        ...
        for i, block in pairs(api.blocks) do
            if block.id == final_target.reference_id then
                target_block = block
                offsetted_block_in_reference_block_pos =
                    0.05 * final_target.offset
                offsetted_block_in_robot_pos =
                    offsetted_block_in_reference_block_pos:rotate(
                        target_block.orientation_robot) +
                        target_block.position_robot
                offsetted_block_in_robot_ori = target_block.orientation_robot
                draw_block_axes(offsetted_block_in_robot_pos,
                                offsetted_block_in_robot_ori, 'blue')
                draw_block_axes(target_block.position_robot,
                                target_block.orientation_robot, 'red')
                break
            end
        end
