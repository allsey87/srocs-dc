...
local create_pickup_block_node = function(target, _forward_distance)
    ...
    return -- return the following table
    {
        ...
        children = {
            ...
            create_reach_block_node(target, _forward_distance), -- touch down
            {
                type = 'selector',
                children = {
                    -- hand full ?
            ...
            }, -- count and raise
            {
                ...
                children = {
                    -- attrack magnet
                    function()
                        robot.electromagnet_system.set_discharge_mode(
                            'constructive')
                            return false,true
                    end, -- wait for 2 sec
                    create_timer_node({
                        time = 2,
                        func = function()
                            print("FIXED WAIT BEFORE LIFT")
                            return true
                        end
                    }), -- raise
                    function()
                        DebugMSG('raising')
                        robot.lift_system.set_position(
                            robot.lift_system.position + 0.05)
                        return false, true -- not running, true
                    end, create_timer_node(
                        {
                            time = 1,
                            func = function()
                                print("FIXED WAIT AFTER LIFT")
                                return true
                            end
                        }), -- recharge magnet
                    function()
            ...
            -- wait
            create_timer_node({time = 2}), function()
                if robot.rangefinders['underneath'].proximity <
                    api.parameters.proximity_touch_tolerance then
                    return false, true -- not running, true
                else
