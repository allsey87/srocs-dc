        ...
        children = {
            function()
                ...
                for i, v in ipairs(target_tag.corners) do
                    if v.x < target_tag.corners.left then
                        target_tag.corners.left = v.x
                    end
                    if v.x > target_tag.corners.right then
                        target_tag.corners.right = v.x
                    end
                    if v.y < target_tag.corners.up then
                        target_tag.corners.up = v.y
                    end
                    if v.y > target_tag.corners.down then
                        target_tag.corners.down = v.y
                    end
                end

                local tolerance = api.parameters.lift_system_position_tolerance
                local target_height = robot.lift_system.position -
                                          target_tag.position.y -- TODO: refine this calculation
                local upper_limit = api.parameters.lift_system_upper_limit
                local lower_limit = api.parameters.lift_system_lower_limit +
                                        target_block.position_robot.z - 0.02
                if target_height < lower_limit then
                    target_height = lower_limit
                end
                if target_height > upper_limit then
                    target_height = upper_limit
                end
                ...
                if aim_point ~= nil and aim_point.case == "left" then
                    if target_tag.corners.left <
                        robot.camera_system.resolution.x * 3 / 32 then
                        turn = "left"
                        flag_orientation = false
                    elseif target_tag.corners.left >
                        robot.camera_system.resolution.x * 5 / 32 then
                        turn = "right"
                        flag_orientation = false
                    else
                        turn = "no"
                ...
                elseif aim_point ~= nil and aim_point.case == "right" then
                    if target_tag.corners.right <
                        robot.camera_system.resolution.x * 27 / 32 then
                        turn = "left"
                        flag_orientation = false
                    elseif target_tag.corners.right >
                        robot.camera_system.resolution.x * 29 / 32 then
                        turn = "right"
                        flag_orientation = false
                    else
                        turn = "no"
                    ...
                    local angle = math.atan(
                                      target_block.position_robot.y /
                                          target_block.position_robot.x) * 180 /
                                      math.pi -- x should always be positive
                ...
                if turn == "left" then
                    DebugMSG("Correct to left")
                    api.move(-api.parameters.default_speed * err -
                                 api.parameters.default_speed / 2,
                             api.parameters.default_speed * err +
                                 api.parameters.default_speed / 2)
                    flag_orientation = false
                elseif turn == "right" then
                    DebugMSG("Correct to right")
                    api.move(api.parameters.default_speed * err +
                                 api.parameters.default_speed / 2,
                             -api.parameters.default_speed * err -
                                 api.parameters.default_speed / 2)
